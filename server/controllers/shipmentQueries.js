...
const getTrackedShipment = async (req, res) => {
  ...
  const shipment = await Shipment.findOne({ _id: req.body._id }).select(
    '-pickUpAddress -weight -deliveryAddress.latitude -deliveryAddress.longitude -deliveryAddress.state -deliveryAddress.country -receiver -sender -payment'
  );
  if (shipment) {
    res.send(shipment);
  } else {
...
const saveShipment = async (req, res) => {
  ...
  if (req.user) {
    ...
    if (req.body.paymentMethod === 'online') shipment.payment = payment._id;
